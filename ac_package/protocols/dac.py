...
- Practical Delegatable Anonymous Credentials From Equivalence Class Signatures, PETS 2023. by Mir et al.
...
from ac_package.zkp import ZKP_Schnorr_FS, Damgard_Transfor
...
class DAC(AC):
    def __init__(self, t, l_message):
        ...
        self.spseq_uc = EQC_Sign(group, t)
        self.setcommit = CrossSetCommitment(group, t)
    ...
    def setup(self):
        ...
        return (pp_dac, proof_vk, vk_stm, sk_ca, proof_alpha, alpha_stm, self.zkp, self.spseq_uc)
    ...
    def nym_gen(self, pp_dac, usk, upk):
        ...
        assert self.zkp.verify(challenge, pedersen_open, pedersen_commit, nym, response)
    ...
    def issue_cred(self, pp_dac, attr_vector, sk, nym_u, k_prime, proof_nym_u):
        ...
        if self.zkp.verify(challenge, pedersen_open, pedersen_commit, stm, response) == True:
            if k_prime != None:
                (sigma, update_key, commitment_vector, opening_vector) = self.spseq_uc.sign(pp_sign, nym_u, sk, attr_vector,
                                                                                       k_prime)
